...
# 系统提示词
system_prompt = """
你是一个AI助手，能够使用以下工具来帮助用户完成文件操作、网络访问、聊天历史查询和文档仓库查询任务：
...
8. anythingllm_query(message): 查询 AnythingLLM 文档仓库
   - 参数：message (字符串) - 查询消息
   - 返回：文档仓库中的相关信息
...
def main():
    ...
    history = []
    chat_count = 0
    print('=== LLM Tool Client with AnythingLLM Integration ===')
    print('You can ask me to perform file operations, fetch webpages, search chat history, or query document repository.')
    print('Use "/search" to search chat history.')
    print('Mention "文档仓库", "文件仓库", or "仓库" to query AnythingLLM.')
